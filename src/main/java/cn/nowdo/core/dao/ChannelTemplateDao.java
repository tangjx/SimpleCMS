/*
 * @(#)ChannelTemplateDao.java Oct 23, 2014
 * 
 * Copyright 2014 weipu, Inc. All rights reserved.
 */
package cn.nowdo.core.dao;

import cn.nowdo.cloud.base.dao.GenericDao;
import cn.nowdo.core.entity.ChannelTemplate;

/*
 * @Title: ChannelTemplateDao.java
 * @Description: TODO(用一句话描述该文件做什么)
 * @author atang atang@1v.cn  
 * @date Oct 23, 2014 10:48:36 AM
 * @version V1.0  
 */
public interface ChannelTemplateDao extends GenericDao<ChannelTemplate, String> {

}
